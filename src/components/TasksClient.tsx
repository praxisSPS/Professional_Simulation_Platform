...
export default function TasksClient({ tasks, activeSession, profile }: Props) {
  ...
  const goodCount  = completed.filter(t => t.decision_quality === 'good').length
  ...
  return (
    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 4, background: '#F1F5F9', borderRadius: 8, padding: 3, width: 'fit-content' }}>
        {([['pending', `Pending (${pending.length})`], ['completed', `Completed (${completed.length})`], ['all', 'All']] as const).map(([t, label]) => (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {shown.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px', color: '#94A3B8', fontSize: 13 }}>
            {tab === 'pending' ? 'No pending tasks — well done, or clock in to receive new ones.' : 'No completed tasks yet.'}
          </div>
        )}
          ...
          return (
            <div
              ...
              style={{
                ...
                borderLeft: task.urgency === 'urgent' && !task.completed_at ? '3px solid #DC2626' : task.completed_at ? '3px solid #86EFAC' : '3px solid transparent',
            ...
            >
              ...
              {isExpanded && (
                <div style={{ borderTop: '0.5px solid #F1F5F9', padding: '12px 14px', background: '#F8FAFC' }}>
                  ...
                  {task.ai_feedback && (
                    <div style={{ background: '#EBF3FB', borderLeft: '3px solid #1F4E79', padding: '10px 12px', borderRadius: '0 8px 8px 0', fontSize: 12, color: '#1E40AF', lineHeight: 1.5, marginBottom: 8 }}>
